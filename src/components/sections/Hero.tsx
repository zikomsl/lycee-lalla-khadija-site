...
export const Hero = () => {
  ...
  return (
    <section id="hero" className="relative min-h-[100svh] w-full overflow-hidden flex flex-col justify-center pt-24 pb-12 bg-white dark:bg-slate-950">
      ...
      <div className="container relative z-10 grid grid-cols-1 md:grid-cols-2 gap-8 md:gap-12 items-center px-6 mx-auto">
        ...
        >
          <motion.div
            ...
            className="inline-flex items-center gap-2 px-3 py-1 rounded-full glass text-[10px] font-sans font-black mb-4 md:mb-6 border border-primary/20 shadow-sm uppercase tracking-widest"
          >
            <Sparkles className="h-3 w-3 text-primary" />
            <span className="text-grad-primary">{t("hero_kicker")}</span>
          </motion.div>

          <StaggerText
            as="h1"
            text={t("hero_title")}
            className="font-sans font-black text-5xl md:text-6xl lg:text-8xl tracking-tight leading-[1.1] text-slate-900 dark:text-white mb-5 md:mb-6"
          />

          <motion.p
            initial={{ opacity: 0 }}
            animate={{ opacity: 1 }}
            className="text-xs md:text-lg text-slate-600 dark:text-slate-400 max-w-[280px] md:max-w-md mx-auto md:mx-0 mb-8 md:mb-10 leading-relaxed font-sans font-bold opacity-90"
          >
            {t("hero_sub")}
          </motion.p>

          <div className="flex justify-center md:justify-start">
            <a href="#branches" className="px-10 py-4 rounded-2xl bg-slate-900 dark:bg-white text-white dark:text-slate-900 text-sm font-sans font-black shadow-xl active:scale-95 transition-transform uppercase tracking-widest">
              {t("hero_cta")}
            </a>
          </div>
        </motion.div>
        ...
        <div className="w-full flex flex-col items-center justify-center overflow-visible z-10 mt-4 md:mt-0">
          ...
          <motion.div
            ...
            className="mt-12 md:-mt-8 glass rounded-full px-6 py-2.5 text-[10px] font-sans font-black flex items-center gap-2 border border-primary/20 shadow-xl bg-white/60 dark:bg-slate-900/60 backdrop-blur-xl z-20 whitespace-nowrap"
          >
      ...
      >
        <span className="text-[9px] uppercase tracking-[0.2em] text-slate-500 font-sans font-black">Découvrir</span>
